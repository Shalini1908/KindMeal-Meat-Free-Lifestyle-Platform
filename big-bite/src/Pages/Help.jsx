...
<Container >
    <Card style={{width:"800px"}} p={2}>
    <Heading fontSize={18} textAlign={'left'}>General Help</Heading>
<Text textAlign={"left"} fontSize={"14px"}  mt={2}>Wish to find out more on how to use KindMeal? Please check out our frequently-asked questions below. If you are unable to find your answer here, please contact us for further assistance.</Text>


<Heading textAlign={'left'} fontSize={18}  mt={2}>What is KindMeal?</Heading>
<Text textAlign={"left"}  fontSize={"14px"} mt={2}>KindMeal is Malaysia's (and possibly the world's!) pioneering meat-free lifestyle platform. We provide an effective platform to promote compassionate dining in a fun and effective way, that would allow you to save precious animal lives, improve your health, enjoy a cleaner environment and of course, save your hard-earned money.</Text>


<Heading textAlign={'left'} fontSize={18}  mt={2}>What is Meat-Free?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>Meat-free means that the food does not contain any meat, or ingredients derived from meat. However, it may contain some elements that people of certain diet do not consume, such as eggs, dairy / milk, or alcohol. Please refer to the details within each specific shop / deal for further information.</Text>


<Heading textAlign={'left'} fontSize={18}  mt={2} >How do I sign up to KindMeal? Is it free?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>Yes, it is absolutely free. To join our community of food lovers and grab awesome deals, please sign up here.


If you are a merchant that wish to promote your business for FREE, please proceed to our Businesses Page.</Text>

<Heading textAlign={'left'} fontSize={18}  mt={2}>What is a Username?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>A Username is a nickname exclusively used to identify your profile. It will be used to access your Public Page, such as https://KindMeal.my/MyUsername, and it will also be displayed when you share KindMoments, comments and reviews.</Text>
...
<Heading textAlign={'left'} fontSize={18}  mt={2}>How do I get a KindMeal coupon?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>Just browse to a deal you like, select the corresponding outlet location (if the restaurant has multiple locations), then click on Get Coupon. Please note that the outlet location cannot be changed after the coupon is obtained.
...
<Image src={about_1}/>
<Heading textAlign={'left'} fontSize={18}  mt={2}>What is KindWords?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>KindWords is a great way to tell your friends about the great food and instantly get more discount.
...
<Heading textAlign={'left'} fontSize={18}  mt={2}>How do I use my KindMeal LiveCoupon?</Heading>
<Text textAlign={"left"} fontSize={"14px"}  mt={2}>Once a coupon is received, you can access the LiveCoupon and use it via one of the following ways. An Internet connection is required.
...
<Heading textAlign={'left'} fontSize={18}  mt={2}>What if I don't have a mobile device or Internet connection, can I use a Print Coupon?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>This depends on the specific deal. If the merchant allows it, you can also choose to print out the coupon and bring it along. Please refer to the deal's Terms & Conditions to check if print coupons are allowed, and if any prior reservation and usage delay are required.
...
<Heading textAlign={'left'} fontSize={18}  mt={2} >What is a Print Coupon's Usage Delay?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>Some merchants may not have the facilities available to verify Print Coupons instantly, so they may impose a Usage Delay of a certain number of days to first print out a cross-checking reference list. If there is a 3-Day delay, it means that you can only use the Print Coupon 3 days after obtaining the coupon.
...
<Heading textAlign={'left'} fontSize={18}  mt={2}>How do I create a shop and offer deals?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>You need to first sign up a merchant account. If you have an existing food lover account, we can upgrade it for you to enable the Merchant functions. To do so, please contact us and tell us more about your shop.

Once your merchant account is activated, you can find out more details at the Merchant Help section.</Text>


<Heading textAlign={'left'} fontSize={18}  mt={2}>Do I need to make any payments to get the deal coupons?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>No, absolutely no payment is required at KindMeal to obtain your coupon. You will only need to make payment at the restaurant when you use the coupon to purchase your food.</Text>


<Heading textAlign={'left'} fontSize={18}  mt={2} >What is a KindMoment?</Heading>
<Text textAlign={"left"}  fontSize={"14px"}  mt={2}>A KindMoment is a food selfie, a photo moment of your delicious food for sharing with the world, and to encourage meat-free dining. Sharing a KindMoment is extremely quick and easy, similar to other social photo sharing websites or mobile apps.
